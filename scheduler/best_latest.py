...
class LLMScheduler:
    """
    调度流程：
    1. 判断任务是否需要拆分（PDF 一定拆；纯文本由 GPT-4o 返回 yes/no）。
    2. 若无需拆分 → GPT-4o 直接生成 Markdown 并返回。
    3. 若需要拆分 → 先推送子任务列表；随后每分配一个子任务就推送一次进度。
    4. 所有 agent 完成后，中心再次汇总并推送最终 Markdown。
    通过 progress_cb(dict) 回调把阶段信息实时发给前端。
    """
    ...
    def dispatch(
        ...
        """
        参数:
            context_id: 会话 id，用于 MemoryStore 归档
            progress_cb: 前端回调函数，形如 lambda dict: ...
        返回:
            {"markdown": 最终结果, "trace": List[...]}

        更新：
            1. 分配任务时，实时返回子任务列表。
            2. 分配子任务后立即返回“Processing”状态，完成后更新为实际结果。
        """
        ...
        if pdf_data:
            ...
            self._push(progress_cb, {"status": "subtasks", "subtasks": page_ids})
        ...
        # -------- 纯文本 / 指令 ----------- 
            ...
            if not self._need_split(task):
                rsp = client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "user", "content": f"请以 Markdown 格式完整回答：\n\n{text.strip()}"},
                    ],
                )
            ...
            self._push(progress_cb, {"status": "subtasks", "subtasks": all_subs})
        ...
        for ag, keys in sub_ids_source.items():
            for key in keys:
                ...
                self._push(progress_cb, {"status": "assign", "agent": ag, "subtask": key, "output": "Processing"})
                payload = {
                    "prompt": prompt_text,
        ...
        rsp = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "user", "content": "请综合以下各智能体输出，总结共识、差异并用 Markdown 返回最终建议：\n" + collected}]
        )
